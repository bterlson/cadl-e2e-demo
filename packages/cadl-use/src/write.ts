...
interface WriterContext {
  ...
  parameterMap: Map<ModelTypeProperty, ParameterInformation>;
  isClient: boolean;
}
...
function normalizeParameters(
  ...
  function parametersOf(operation: OperationDetails) {
    ...
    if (operation.parameters.body)
      theseParameters.push(operation.parameters.body);
...
export function writeClientFile(
  program: Program,
  type: NamespaceType | InterfaceType
): string {
...
export function writeOperationFile(
  ...
  if (details.parameters.body) {
    context.parameterMap.set(details.parameters.body, {
      disposition: "operation",
      location: details.parameters.body.name,
      typeName: convertToTypeScript(
        context,
        details.parameters.body.type,
...
function convertToTypeScript(
  ...
  switch (t.kind) {
    ...
    case "Array":
      return `${convertToTypeScript(context, t.elementType)}[]`;
    case "Model":
...
function addInterface(
  ...
  const t = context.program.checker.getEffectiveModelType(tOriginal);
...
function addEnum(context: WriterContext, e: EnumType): string {
